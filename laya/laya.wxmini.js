...
*/
    //class laya.wx.mini.MiniLocation
        ...
        MiniLocation.watchPosition = function(success, error, options) {
            ...
            Laya.timer.loop(1e3, null, MiniLocation._myLoop);
        };
        ...
        MiniLocation._myError = function(res) {
            ...
            for (key in MiniLocation._watchDic) {
                if (MiniLocation._watchDic[key].error) {
                    MiniLocation._watchDic[key].error(/*no*/ this.rst);
                }
            }
        };
    ...
    //class laya.wx.mini.MiniFileMgr extends laya.events.EventDispatcher
            ...
            var downloadTask = MiniFileMgr.wxdown({
                ...
                success: function(data) {
                    if (data.statusCode === 200) {
                        ...
                        //就算载不到微信也会返回success,但是文件是404,这样会卡住loader
                        if (data.statusCode === 404) {
                            console.log("文件404:", fileUrl);
                        }
                        callBack.runWith([ 1, data ]);
                    }
                },
                fail: function(data) {
                    callBack != null && callBack.runWith([ 1, data ]);
        ...
        MiniFileMgr.copyFile = function(tempFilePath, readyUrl, callBack) {
            ...
            if (fileUseSize + chaSize >= totalSize) MiniFileMgr.onClearCacheRes(5 * 1024 * 1024);
            MiniFileMgr.fs.copyFile({
                ...
                success: function(data) {
                    if (!fileObj) {
                        ...
                        if (fileObj.readyUrl != readyUrl) MiniFileMgr.remove(tempFileName, readyUrl, callBack); else callBack != null && callBack.runWith([ 0 ]);
                    }
                },
                fail: function(data) {
                    callBack != null && callBack.runWith([ 1, data ]);
        ...
        MiniFileMgr.onClearCacheRes = function(memSize) {
            ...
            for (var key in MiniFileMgr.filesListObj) {
                ...
                if (key != "fileUsedSize") {
                    if (clearFileSize >= memSize) break;
        ...
        MiniFileMgr.remove = function(tempFileName, readyUrl, callBack) {
            readyUrl === void 0 && (readyUrl = "");
            readyUrl = window.replacePath(readyUrl);
            var fileObj = MiniFileMgr.getFileInfo(readyUrl);
            if (fileObj == null) {
                return;
            }
